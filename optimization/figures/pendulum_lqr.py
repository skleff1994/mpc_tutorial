...
ax1.set_title("LQR + Inequality Constraints = QP", fontsize=20)

# States
ax1.plot(time_x, xs[:,0], '--', linewidth=3, label=r"$x_1$ (position)", color="#41C29B")
ax1.plot(time_x, xs[:,1], '--', linewidth=3, label=r"$x_2$ (velocity)", color="#82B3DB")
ax1.set_ylabel('State', fontsize=18)
ax1.grid()
ax1.legend(fontsize=18, loc='lower right')

# Velocity constraint |x2| <= 1
ax2.fill_between(time_u, u_max, 3, color="#FF783C", alpha=0.2, label="Infeasible Region")
ax2.fill_between(time_u, -u_max, -3, color="#FF783C", alpha=0.2)
# ax2.fill_between(x1, -2, -1, color="#FF783C", alpha=0.2)
...
# Controls
ax2.step(time_u, us.flatten(), where='post', linewidth=3, color="#2412EB", label=r"$u$ (control input)")
ax2.axhline(u_max, linestyle='--', color="#FF783C", linewidth=2, label=r"$u_{max}$")
ax2.axhline(u_min, linestyle='--', color="#FF783C", linewidth=2)
...
# problem = crocoddyl.ShootingProblem(x0, [lqr_model]*T, lqr_model)
